...
1. Phase 0 完了（テスト土台）
2. Phase 1 完了（自動付与が本番運用可能に）→ ここまでで「手動シード脱却」の実利
3. Phase 2 完了（年5日管理）→ コンプライアンス上の最大価値
4. Phase 3 完了（管理簿出力）
5. Phase 4 完了（時間単位年休）
6. Phase 5 完了（期間一括申請）→ Phase 0 完了後であれば他Phaseと独立に着手可能。既存の承認処理の並行性バグ修正も含む

---

## 次アクション候補

- Phase 0＋Phase 1 を1つの作業単位として着手するのがおすすめ（土台＋最初の実利）。
...
- 付与バッチの実行方式（手動/スケジュール/両方）
- 時間単位年休の「1日=時間数」の基準値
- 時間単位年休で同日複数申請を許容するか（部分ユニーク制約の設計に影響）
